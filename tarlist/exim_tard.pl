...
# Modified for TARLIST functionality by Nathan Butcher
#
# The problem:
#
...
my $sockname = "/var/run/exim_tard.sock";
my $pidfile = "/var/run/exim_tard.pid";

my %cmds = (
	"TARLIST" => \&Tarlist::lookup,
);
...
if (open(F, "<$pidfile"))
{
    ...
    close(F);
    chop($running);
    die "Another exim_tard is running (pid = $running)\n"
        if (-e "/proc/$running");
}
